...
problema no rmagick => http://stackoverflow.com/questions/3704919/installing-rmagick-on-ubuntu

create db with sudo -u postgres psql, and create user xxx with password yyy, and create database cata_xxx owner xxx, grant all on database 	cata_xxx to xxx

rake db:migrate com erro - > grant superuser status -> alter role <user name> with superuser
